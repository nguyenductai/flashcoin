...
#if defined(NDEBUG)
# error "Coolcash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock(CONF_GENESIS_BLOCK);
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Coolcash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Coolcash Signed Message:\n";
...
{
    // Coolcash: IsDust() detection disabled, allows any valid dust to be relayed.
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    // ntr@21092014 - Temporary remove transaction fee (hack code)
    nMinFee = 0;
    return nMinFee;
}
...
//    int64 nSubsidy = 50 * COIN;
    int64 nSubsidy = CONF_REWARD_COIN_VALUE * COIN;
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    //nSubsidy >>= (nHeight / 840000); // Coolcash: 840k blocks in ~4 years
    
    if(nHeight > CONF_NUMBER_BLOCK_HAS_REWARD) // no block reward after 3000 blocks
        nSubsidy = 0;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coolcash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coolcash: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Coolcash: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256(CONF_GENESIS_BLOCK_TESTNET);
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = CONF_BLOCK_NVERSION;
        block.nTime    = CONF_BLOCK_NTIME;
        block.nBits    = CONF_BLOCK_NBITS;
        block.nNonce   = CONF_BLOCK_NNONCE;

        if (fTestNet)
        {
            block.nTime    = CONF_BLOCK_NTIME_TESTNET;
            block.nNonce   = CONF_BLOCK_NNONCE_TESTNET;
        }
        ...
        assert(block.hashMerkleRoot == uint256(CONF_BLOCK_HASH_MERKLE_ROOT));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Coolcash: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GoldbitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GoldbitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GoldbitMiner(CWallet *pwallet)
{
    printf("GoldbitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Coolcash-miner");
    ...
    try { loop {
        ...
        printf("Running GoldbitMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GoldbitMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GoldbitMiner, pwallet));
}
